...
export function WhatsAppAbsenceModal({
  ...
  // Track sent status during current session with localStorage persistence
  const storageKey = `gm_sent_absence_whatsapp_${selectedDate}`;
  const [sentStudentIds, setSentStudentIds] = useState<Record<string, boolean>>({});

  useEffect(() => {
    if (typeof window !== 'undefined') {
      try {
        const saved = localStorage.getItem(`gm_sent_absence_whatsapp_${selectedDate}`);
        if (saved) {
          setSentStudentIds(JSON.parse(saved));
        }
      } catch (e) {
        console.error('Error loading sent absence WhatsApp status:', e);
      }
    }
  }, [selectedDate]);
  ...
  const markAsSent = (studentId: string, isSent: boolean = true) => {
    setSentStudentIds((prev) => {
      ...
      if (typeof window !== 'undefined') {
        try {
          localStorage.setItem(`gm_sent_absence_whatsapp_${selectedDate}`, JSON.stringify(updated));
        } catch (e) {
          console.error(e);
        }
      }
    ...
    if (isSent) {
      logAuditEvent({
        action: 'ABSENCE_WHATSAPP_NOTIFICATION_SENT',
  ...
  const handleResetSentSession = () => {
    ...
    if (typeof window !== 'undefined') {
      try {
        localStorage.removeItem(`gm_sent_absence_whatsapp_${selectedDate}`);
      } catch (e) {
        console.error(e);
      }
    }
  ...
  const handleQuickSend = (student: Student, record?: StudentAttendance) => {
    ...
    markAsSent(student.id, true);

    notify({
      title: dir === 'rtl' ? 'تم فتح محادثة واتساب وتحديث العداد' : 'WhatsApp Ouvert',
      message: dir === 'rtl'
        ? `تم تجهيز رسالة غياب التلميذ ${student.first_name} ${student.last_name} وتحديث العداد بنجاح.`
        : `Message WhatsApp préparé pour ${student.first_name} ${student.last_name}. Compteur mis à jour.`,
      type: 'success',
    });
  };
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-1.5 shrink-0 self-end sm:self-center">
                    ...
                    <button
                      type="button"
                      onClick={() => markAsSent(student.id, !isSent)}
                    ...
                    <button
                      ...
                      className="p-2 rounded-xl text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 transition-colors cursor-pointer border border-slate-200 dark:border-slate-700"
                    ...
                    <button
                      ...
                      className={`inline-flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold text-white shadow-md transition-all cursor-pointer transform active:scale-95 ${
                        normalized

...
export default function StaffAttendancePage() {
    ...
    try {
      ...
      const teacherRecords = newRecords
        ...
        .map((r) => ({
          ...
          notes: r.notes,
          reason: r.justification_reason || r.notes || (r.late_minutes > 0 ? `Retard: ${r.late_minutes}m` : null),
        }));
  ...
  const handleExportDailyCSV = () => {
    ...
    link.setAttribute('download', `Pointage_Personnel_${selectedDate}.csv`);
                        ...
                        <tr key={staff.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors">
                          ...
                          <td className="py-2.5 px-3 text-center">
                            <div className="inline-grid grid-cols-4 gap-0.5 p-0.5 rounded-xl bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700/80 w-full max-w-[250px]">
                              ...
                              <button
                                type="button"
                                onClick={() => handleQuickStatusChange(staff, 'EXCUSED')}
                                className={`px-1 py-1 rounded-lg text-[10px] font-bold truncate transition-all cursor-pointer ${
                                  status === 'EXCUSED'
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-[10pt] font-bold text-slate-700 mt-0.5">
                  FEUILLE OFFICIELLE DE POINTAGE ET D&apos;ASSIDUITÉ DU PERSONNEL
                </p>
